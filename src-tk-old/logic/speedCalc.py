...
def rB_calcs(aBT, hB, G, TF, v)->float:
    ...
    p_B = 0.56 * math.sqrt(aBT) / (TF - (1.5 + 0.000001)* hB)
...
def calcHPforSpeed(): #TODO: extract magic numbers
    ...
    beam = ship.hull.beam
    draft = ship.hull.draft
    displacement = ship.hull.displacement
    numShafts = ship.hull.shaftCount
    numBlades = 4
    speed = ship.speed / 1.944
    cM = 0.95
    cWP = 0.7
    dProp = 3.5
    propSpeed = 3
